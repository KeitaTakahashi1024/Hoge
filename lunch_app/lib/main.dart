...
class pdfSetState extends State<pdfSet> {
  ...
  Future<File> getFileFromUrl(String url, int counterMenu, int counterPath) async {
    ...
    try {
      ...
      //print(dir.path);
      if(counterMenu > 9) {
        count = counterMenu ~/ 10;
        counterMenu = counterMenu - count * 10;
      }
      File file =
  ...
  Widget _buildBody() {
    return SingleChildScrollView(
      child: Column(
        children: <Widget>[
          _titleText('だしを活用した和食メニュー'),
          _gridList(allMenuName[0], allMenuImage[0], 0),
          _titleText('各調理場自慢のオリジナルレシピ '),
          _gridList(allMenuName[1], allMenuImage[0], 1),
          _titleText('スチームコンベクションオーブンを使ったメニュー '),
